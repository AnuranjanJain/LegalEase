...
export function ChatbotPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(
        `Document "${data.filename}" context integrated successfully!`,
        'success'
      );
  ...
  const handleSummarize = async () => {
    ...
    try {
      const data = await api.post<{ summary: string }>('/summarize', {
      ...
      <div className="flex-grow overflow-y-auto px-6 py-8 space-y-6 relative z-10">
        {messages.map((msg: ChatMessage) => {
          ...
          return (
            ...
            >
              <div
                className={`flex items-start max-w-[80%] gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}
              >
                ...
                {/* Message Bubble Card */}
                <div
                  className={`p-4 rounded-2xl shadow-sm text-left leading-relaxed ${
                    isUser
                ...
                >
                  ...
                  <p
                    className={`text-[9px] font-semibold mt-2 ${isUser ? 'text-blue-100 text-right' : 'text-gray-400 dark:text-gray-500'}`}
                  >
                    {msg.time}
                  </p>
                </div>
